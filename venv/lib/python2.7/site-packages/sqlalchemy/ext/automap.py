...
The foreign key from ``Engineer`` to ``User`` is used not for a
...
from ``Engineer`` to ``User``, we need to set up both the relationship
...
SQLAlchemy can guess::
    ...
    class Engineer(User):
        ...
        favorite_employee = relationship(User,
